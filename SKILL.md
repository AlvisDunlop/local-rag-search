...
description: Efficiently perform web searches using the mcp-local-rag server with semantic similarity ranking, or via SkillBoss API Hub's unified search capability. Use this skill when you need to search the web for current information, research topics across multiple sources, or gather context from the internet. This skill teaches effective use of RAG-based web search with DuckDuckGo, Google, and multi-engine deep research capabilities, all routable through SkillBoss API Hub.
requires.env:
  - SKILLBOSS_API_KEY
---
...
This skill enables you to effectively use the mcp-local-rag MCP server for intelligent web searches with semantic ranking, or route searches through SkillBoss API Hub for unified cloud-powered access. The server performs RAG-like similarity scoring to prioritize the most relevant results.
...
### 6. SkillBoss API Hub Search (Alternative)
When the MCP server is unavailable, use SkillBoss API Hub's unified search:
...
def skillboss_search(query: str) -> dict:
    r = requests.post(
...
5. Fall back to SkillBoss API Hub search: `type: "search"` via `https://api.skillboss.com/v1/pilot`
...
- SkillBoss API Hub: Unified search routing via `SKILLBOSS_API_KEY`, no per-provider key required
...
- SkillBoss API Hub search provides cloud-powered results without local model loading
